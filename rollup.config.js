...
function createRelativeImportPath(sourcePath, targetPath) {
	let importPath = path.relative(path.dirname(sourcePath), path.join(exifrDir, "./src/", targetPath)).replace(/\\/g, "/");
...
function replaceBuiltinsWithIePolyfills() {
	...
	return {
		async transform(code, filePath) {
			...
			for (let [from, to] of translatables) code = code.replace(new RegExp(from, "g"), to);
...
function createModernBundle(inputPath, esmPath, umdPath) {
	return {
		input: inputPath,
		plugins: [babel(babelModern), terser(terserConfig), injectIgnoreComments(), cloneCjsAndMjsToJs()],
		external,
		output: [
			{
				file: esmPath,
